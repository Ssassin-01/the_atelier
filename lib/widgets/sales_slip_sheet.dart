...
class _SalesSlipSheetState extends ConsumerState<SalesSlipSheet> {
  ...
  void initState() {
    ...
    if (widget.initialTransaction != null) {
      ...
      final convertedPrice = (tx.amount / qty) * settings.exchangeRate;
      entry.amountController.text = convertedPrice.toStringAsFixed(settings.currencyFormat.decimalDigits ?? 0);
      _items.add(entry);
    } else {
  ...
  void _removeItem(int index) {
    if (_items.length > 1) {
      ...
      _listKey.currentState?.removeItem(
        index,
        (context, animation) => _buildEntryRow(
          index,
          animation,
          isRemoving: true,
          itemOverride: removedItem,
        ),
        duration: const Duration(milliseconds: 300),
      );
  ...
  Future<void> _handleSave() async {
    ...
    for (int i = 0; i < validItems.length; i++) {
      ...
      final tx = BusinessTransaction(
        ...
        type: _currentType,
        amount: (qty * price) / settings.exchangeRate,
        category: _currentType == 'sale'
            ? l10n.productSale
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _currentType == 'sale' ? l10n.saleRegistered : '기록되었습니다',
          ),
          backgroundColor: _currentType == 'sale'
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                            "${_currentType == 'sale' ? '+' : '-'}${settings.format(total)}",
                            style: ArtisanalTheme.receipt(
                              fontSize: 20,
                              fontWeight: FontWeight.bold,
                              color: _currentType == 'sale'
                                  ? ArtisanalTheme.greenInk
                                  : Colors.red.shade700,
                            ),
                          ),
                      ...
                      Center(child: _buildPaidStampButton()),
                      if (widget.initialTransaction != null) ...[
              ...
              Expanded(
                ...
                child: Autocomplete<Recipe>(
                  ...
                  onSelected: (recipe) {
                    ...
                    final settings = ref.read(settingsProvider);
                    final convertedPrice = costData.suggestedPrice * settings.exchangeRate;
                    entry.amountController.text = convertedPrice
                        .toStringAsFixed(settings.currencyFormat.decimalDigits ?? 0);
                    setState(() {});
                  },
                  fieldViewBuilder:
                      (context, controller, focusNode, onFieldSubmitted) {
                        if (entry.descriptionController.text.isNotEmpty &&
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                title: Text(
                                  option.name,
                                  style: ArtisanalTheme.receipt(fontSize: 14),
                                ),
              ...
              Row(
                children: [
                  _buildQtyButton(Icons.remove, () {
                    final current =
                        int.tryParse(entry.quantityController.text) ?? 1;
                    if (current > 1) {
                  ...
                  _buildQtyButton(Icons.add, () {
                    final current =
                        int.tryParse(entry.quantityController.text) ?? 0;
                    entry.quantityController.text = (current + 1).toString();
                    setState(() {});
                  }),
                ],
            ...
            children: [
              AnimatedOpacity(
                ...
                child: Text(
                  (widget.initialTransaction != null
                          ? '수정 완료'
                          : (_currentType == 'sale' ? l10n.paid : '지출 확인'))
                      .toUpperCase(),
                  style: ArtisanalTheme.hand(
                    fontSize: 24,
                    fontWeight: FontWeight.bold,
